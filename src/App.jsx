...
import { useAccount, useLogin, useLogout } from '@windsurf/react-auth';

function App() {
  const { user, isLoading: authLoading } = useAccount();
  ...
  useEffect(() => {
    // Load saved graphs from localStorage or Windsurf user metadata
    if (user) {
      const saved = localStorage.getItem(`savedGraphs_${user.id}`);
      if (saved) {
  ...
  const saveGraph = (url) => {
    ...
    localStorage.setItem(`savedGraphs_${user.id}`, JSON.stringify(newGraphs));
  };
